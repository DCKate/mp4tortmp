...
void* RtmpRoutine (void* arg){
    ...
    sleep(5);
    while(keep_live){
        knode* node = pop_kqueue_node(mediaQueue);
        if (node)
        {
            switch(node->type){
                case VIDEO_FRAME:{
    ...
    *got_frame = 0;
    ...
    if (pkt.stream_index == video_stream_idx) {
        ...
        //printf("decode pts [%d] size %d\n", pkt.pts,pkt.size);
         while((ret = avcodec_receive_frame(video_dec_ctx, frame))>=0){
            ...
            while((ret = avcodec_receive_packet(video_enc_ctx, &encpkt))>=0){
                ...
                push_kqueue(mediaQueue, (encpkt.data),VIDEO_FRAME, encpkt.size);
        ...
        if (*got_frame)
        {
            ...
            ret = avcodec_receive_packet(video_enc_ctx, &encpkt);
        ...
        if (*got_frame) {
			...
			if (aac_size > 0) {
                ...
                push_kqueue(mediaQueue, audio_aac_out,AUDIO_FRAME,aac_size);
			}
...
int main(int argc, char const *argv[])
{
    ...
    pthread_create (&pid_rtmp, NULL, RtmpRoutine, NULL);
